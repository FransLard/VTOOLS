/* VelardTools merge menu — gabung 2+ video jadi 1 file.
 * Pola clone dari thumbnail.js: scoped ke #toolPanel-merge, guard anti double-init,
 * i18n via window.i18n.t + fallback ID, notify via #appToast fallback alert.
 * Encode via window.velardFFmpeg.load() (bridge ke loader FFmpeg utama, tanpa duplikasi init).
 * Strategi encode (mulus, anti patah di sambungan): tiap input dinormalisasi
 * (CFR 30fps, PTS dari 0, audio AAC seragam/sunyi) lalu concat demuxer +genpts. */
(function () {
  ...
  function mergeNotify(msg) {
    try {
      ...
      if (toast) {
        ...
        mergeNotify._t = setTimeout(function () { toast.classList.remove("show"); }, 3500);
        return;
      }
    } catch (e) { /* ignore */ }
  ...
  var files = []; // {id, file, name, size, duration, width, height, url, thumbUrl}
  var uid = 0;
  var busy = false;
  window.__mergeBusy = false;
  function setBusy(v) { busy = v; window.__mergeBusy = v; }
  ...
  function showWarn(msg) {
    if (!warnEl) { if (msg) mergeNotify(msg); return; }
    if (!msg) { warnEl.textContent = ""; warnEl.hidden = true; return; }
  ...
  /* Resolusi output = resolusi terbesar dari input (sisi panjang max 1080p),
   * supaya tidak burem. Bukan 720p paksa. */
  function targetSize() {
    var bw = 0, bh = 0;
    files.forEach(function (f) {
      if (f.width > 0 && f.height > 0 && f.width * f.height > bw * bh) { bw = f.width; bh = f.height; }
    });
    if (!(bw > 0 && bh > 0)) { bw = 1280; bh = 720; }
    var sc = Math.min(1, 1920 / Math.max(bw, bh));
    var W = Math.round(bw * sc / 2) * 2, H = Math.round(bh * sc / 2) * 2;
    return { w: Math.max(2, W), h: Math.max(2, H) };
  }
  function fmtBytes(n) {    if (!isFinite(n) || n < 0) return "-";
  ...
  function refreshState() {
    ...
    var total = 0, ok = true;
    files.forEach(function (f) { total += (f.duration || 0); if (!f.duration) ok = false; });
    if (totalEl) {
      totalEl.textContent = n
        ? mergeT("toolMergeCount", "{n} video").replace("{n}", String(n)) + " • " + fmtDur(total) + (ok ? "" : " • " + mergeT("toolMergeReading", "membaca…"))
        : "";
    }
    if (runBtn) {
  ...
  function previewFile(f) {
    if (!preview || !f) return;
    selectedId = f.id;
    // JANGAN revoke preview.src di sini: itu URL blob milik klip (files[].url).
    // Revoke hanya saat klip dihapus/clear, kalau tidak klip yang dikunjungi
    // ulang jadi URL mati -> video gelap abu-abu tidak tampil apa-apa.
    preview.src = f.url;
    try { preview.load(); } catch (e) {}
    // pratinjau ikut status bisu klip: efek mute langsung terdengar, bukan cuma saat encode
    try { preview.muted = !!f.muted; } catch (e) {}
  ...
  function totalDuration() {
    var t = 0;
    files.forEach(function (f) { t += (f.duration || 0); });
    return t;
  }
  function offsetOf(id) {
    // detik kumulatif semua klip SEBELUM klip ini (jam virtual ala CapCut)
    var off = 0;
  ...
  function updateTime() {
    ...
    try {
      if (preview) {
        // jam berjalan = offset klip + posisi di dalam klip (tidak restart per klip)
  ...
  function currentFile() {
    for (var k = 0; k < files.length; k++) { if (files[k].id === selectedId) return files[k]; }
    return files[0] || null;
  }
  function stepClip(dir) {
    if (!files.length || busy) return;
    var i = 0;
    for (var k = 0; k < files.length; k++) { if (files[k].id === selectedId) { i = k; break; } }
  ...
  function detectMeta(f) {
    return new Promise(function (resolve) {
      ...
      try {
        ...
        v.preload = "metadata";
        v.muted = true;
        var timer = setTimeout(function () { cleanup(); fin(); }, 8000);
        function cleanup() { clearTimeout(timer); v.removeAttribute("src"); try { v.load(); } catch (e) {} }
        v.onloadedmetadata = function () {
          f.duration = isFinite(v.duration) ? v.duration : 0;
          f.width = v.videoWidth || 0;
          f.height = v.videoHeight || 0;
          cleanup(); fin(); render(); refreshState();
        };
        v.onerror = function () { cleanup(); fin(); render(); refreshState(); };
        v.src = f.url;
      } catch (e) { fin(); }
    });
  }
  ...
  function addFiles(list) {
    ...
    for (var i = 0; i < arr.length; i++) {
      ...
      if (SUPPORTED_EXT.indexOf(ext) === -1 && !typeOk) { skipped++; continue; }
      if (file.size > MAX_BYTES) { skipped++; continue; }
      // file yang SAMA boleh digabung (mis. loop/gandakan): tiap entri
      // dapat URL blob & nama input sendiri, jadi aman.
      var item = {
        id: ++uid, file: file, name: file.name || ("video-" + uid + ".mp4"),
        size: file.size || 0, duration: 0, width: 0, height: 0,
        url: objUrl(file)
      };
      files.push(item);
      detectMeta(item);
    ...
    if (files.length && preview && !preview.src) previewFile(files[0]);
    else if (files.length && preview && preview.src === "") previewFile(files[0]);
    render();
    refreshState();
  }
  window.velardMergeAdd = addFiles;

  /* Filmstrip ala CapCut: tangkap beberapa frame tiap klip via canvas (async, guarded) */
  function makeStrip(f) {
    ...
    try {
      ...
      var N = 4;
      var shots = [];
      var timer = setTimeout(function () { cleanup(); f._stripBusy = false; }, 12000);
      function cleanup() {
        clearTimeout(timer);
      ...
      v.onloadedmetadata = function () {
        ...
        if (!dur) { cleanup(); f._stripBusy = false; return; }
        var idx = 0;
        v.onseeked = function () {
          try {
          ...
          idx++;
          if (idx < N) {
            try { v.currentTime = Math.min(dur - 0.1, dur * (0.12 + 0.76 * idx / (N - 1))); } catch (e) { next(); }
          } else { next(); }
        };
        function next() { cleanup(); f.strips = shots; f._stripBusy = false; render(); }
        v.onerror = next;
        try { v.currentTime = Math.min(dur - 0.1, dur * 0.12); } catch (e) { next(); }
      };
      v.onerror = function () { cleanup(); f._stripBusy = false; };
      v.src = f.url;
    } catch (e) { f._stripBusy = false; }
  }
  ...
  function renderRuler(total) {
    ...
    // garis vertikal tiap 1 detik ala After Effects via gradient (murah, tajam),
    // garis 5 detik lebih terang; label teks adaptif agar tidak berdesakan.
    var w = 0;
    try { w = rulerEl.clientWidth || rulerEl.parentElement.clientWidth || 300; } catch (e) { w = 300; }
    var px = Math.max(2, w / total);
    rulerEl.style.background =
      "repeating-linear-gradient(90deg, rgba(255,255,255,.30) 0 1px, transparent 1px " + px + "px)," +
      "repeating-linear-gradient(90deg, rgba(255,255,255,.55) 0 1px, transparent 1px " + (px * 5) + "px)," +
      "rgba(255,255,255,.02)";
    var steps = [1, 2, 5, 10, 15, 30, 60, 120];
    var step = steps[steps.length - 1];
    for (var s = 0; s < steps.length; s++) {
      if (total / steps[s] <= 40) { step = steps[s]; break; }
    }
    for (var t = 0; t <= total + 0.001; t += step) {
  ...
  function virtualNow() {
    ...
    try { if (preview) cur = isFinite(preview.currentTime) ? preview.currentTime : 0; } catch (e) {}
    return off + cur;
  }
  function updatePlayhead() {
    ...
    if (!(total > 0) || !files.length) { ph.hidden = true; return; }
  ...
  function startLoop() {
    try { if (rafId === null && preview && !preview.paused && typeof requestAnimationFrame === "function") rafId = requestAnimationFrame(tickLoop); } catch (e) {}
  }
  function stopLoop() {
    try { if (rafId !== null && typeof cancelAnimationFrame === "function") cancelAnimationFrame(rafId); } catch (e) {}
    rafId = null;
  }

  /* Klik detik di ruler ala After Effects: cari klip pemilik detik itu lalu seek */
  function seekVirtual(sec) {
    ...
    for (var k = 0; k < files.length; k++) {
      ...
      if (sec < acc + d || k === files.length - 1) { target = files[k]; off = acc; break; }
      acc += d;
    }
  ...
  function render() {
    ...
    renderRuler(files.length ? files.reduce(function (a, f) { return a + (f.duration || 0); }, 0) : 0);
    files.forEach(function (f, idx) {
      ...
      mb.addEventListener("click", function (e) {
        ...
        if (busy) return;
        f.muted = !f.muted;
        // bila klip ini sedang tampil, langsung bisukan/suarakan pratinjau
        try { if (preview && f.id === selectedId) preview.muted = !!f.muted; } catch (err) {}
        mergeNotify(f.muted
          ? mergeT("toolMergeMutedToast", "Klip dibisukan. Berlaku di hasil gabungan.")
          : mergeT("toolMergeUnmutedToast", "Klip disuarakan kembali."));
        render();
        refreshState();
      });
      clip.appendChild(mb);

      clip.addEventListener("click", function () { previewFile(f); });
      clip.addEventListener("dragstart", function (e) {
        if (busy) { e.preventDefault(); return; }
        dragId = f.id;
        clip.classList.add("dragging");
        try { e.dataTransfer.effectAllowed = "move"; e.dataTransfer.setData("text/plain", String(f.id)); } catch (err) {}
      });
      clip.addEventListener("dragend", function () { dragId = null; clip.classList.remove("dragging"); });
      clip.addEventListener("dragover", function (e) {
        if (busy || dragId === null || dragId === f.id) return;
        e.preventDefault();
        clip.classList.add("dragover");
      });
      clip.addEventListener("dragleave", function () { clip.classList.remove("dragover"); });
    ...
    listEl.ondragover = function (e) { if (!busy && dragId !== null) e.preventDefault(); };
    listEl.ondrop = function (e) {
      e.preventDefault();
      if (busy || dragId === null || !files.length) return;
      reorderTo(dragId, files[files.length - 1].id, true);
    };
    if (files.length && selectedId === null) previewFile(files[0]);
    else if (files.length && preview && previewName) {
      var cur = currentFile();
  ...
  function reorderTo(fromId, toId, toEnd) {
    ...
    if (toEnd) files.push(item);
    else files.splice(to, 0, item);
    render();
    refreshState();
  }
  function removeFile(id) {
    ...
    for (var k = 0; k < files.length; k++) {
      if (files[k].id === id) {
        try { URL.revokeObjectURL(files[k].url); } catch (e) {}
        files.splice(k, 1);
        break;
      }
    }
    if (selectedId === id) {
      selectedId = null;
      // klip yang sedang tampil ikut dibuang -> kosongkan monitor
      try { if (preview) { preview.removeAttribute("src"); preview.load(); } } catch (e) {}
  ...
  function clearAll() {
    if (busy) return;
    try { if (preview) preview.pause(); } catch (e) {}
    files.forEach(function (f) { try { URL.revokeObjectURL(f.url); } catch (e) {} });
    files = [];
    selectedId = null;
    try { if (fileInput) fileInput.value = ""; } catch (e) {}
    if (preview) { try { preview.removeAttribute("src"); preview.load(); } catch (e) {} }
    if (previewName) previewName.textContent = "";
    showWarn(null);
    setStatus("");
    hideProgress();
  ...
  function safeUnlink(ffmpeg, names) {
    (names || []).forEach(function (n) {
      try { if (ffmpeg.deleteFile) ffmpeg.deleteFile(n); } catch (e) { /* ignore */ }
    });
  }
  ...
  async function runMerge() {
    ...
    try {
      ...
      try {
        if (ffmpeg.on) ffmpeg.on("progress", function () { /* status via tahap */ });
      } catch (e) {}
      var stamp = Date.now();
      var inputs = [];
      ...
      // Normalisasi tiap input dulu (wajib mulus): samakan fps/CFR, reset PTS ke 0,
      // samakan audio (sunyi bila input tak ada audio), lalu concat demuxer +genpts.
      // Concat -c copy mentah TIDAK dipakai: timestamp diskontinu = patah-patah di sambungan.
      var THREADS = "2";
      try { THREADS = String(Math.min(4, (navigator.hardwareConcurrency || 2))); } catch (e) {}
      var logBuf = null;
      var progBase = 0, progSpan = 0; // jendela progress untuk encode berjalan
      try {
        if (ffmpeg.on) {
          ffmpeg.on("log", function (ev) { if (logBuf && ev && ev.message) logBuf.push(ev.message); });
      ...
      async function hasAudioStream(inName) {
        logBuf = [];
        try { await ffmpeg.exec(["-hide_banner", "-i", inName]); } catch (e) { /* exit!=0 wajar: tanpa output */ }
        var out = logBuf || [];
        logBuf = null;
        return out.some(function (m) { return /Stream #\d+:\d+[^:]*: Audio/.test(m); });
      }
      var TS = targetSize();
      ...
      for (var j = 0; j < inputs.length; j++) {
        ...
        progBase = base; progSpan = 0; // probe: jangan gerak liar
        if (withAudio) {
          args = ["-y", "-i", inputs[j], "-threads", THREADS,
            "-vf", VF, "-af", AF, "-r", "30",
            "-c:v", "libx264", "-preset", "veryfast", "-crf", "20", "-pix_fmt", "yuv420p",
            "-ar", "44100", "-ac", "2", "-c:a", "aac", "-b:a", "128k",
            "-movflags", "+faststart", nName];
        } else {
          args = ["-y", "-i", inputs[j], "-f", "lavfi", "-i", "anullsrc=r=44100:cl=stereo", "-threads", THREADS,
            "-map", "0:v", "-map", "1:a",
            "-vf", VF, "-r", "30",
            "-c:v", "libx264", "-preset", "veryfast", "-crf", "20", "-pix_fmt", "yuv420p",
            "-ar", "44100", "-ac", "2", "-c:a", "aac", "-b:a", "128k",
            "-shortest", "-movflags", "+faststart", nName];
        }
        progSpan = span; // encode jalan: progress FFmpeg menggerakkan bar
        await ffmpeg.exec(args);
        progBase = 0; progSpan = 0;
        // bebaskan RAM input mentah segera (file besar x N bisa bikin macet)
      ...
      await ffmpeg.writeFile(list2, new TextEncoder().encode(norms.map(function (n) { return "file '" + n + "'"; }).join("\n")));
      created.push(list2);
      // +genpts: bangun ulang timestamp di sambungan agar tidak patah-patah
      setProgress(88, mergeT("toolMergeStep4", "Menggabungkan…"));
      progBase = 88; progSpan = 6;
      ...
      var blob = data instanceof Blob ? data : new Blob([data], { type: "video/mp4" });
  ...
  // hook baca-saja untuk regression test (jam virtual kumulatif)
  window.__mergeClock = { total: totalDuration, offset: offsetOf, seek: seekVirtual };
  ...
  if (fileInput) fileInput.addEventListener("change", function () { addFiles(fileInput.files); try { fileInput.value = ""; } catch (e) {} });  if (drop) {
  ...
  if (prevBtn) prevBtn.addEventListener("click", function () { stepClip(-1); });
  if (nextBtn) nextBtn.addEventListener("click", function () { stepClip(1); });
  ...
  if (preview) {
    ...
    preview.addEventListener("play", function () { setPlayIco(true); startLoop(); });
    preview.addEventListener("pause", function () { setPlayIco(false); stopLoop(); updateTime(); });
    ...
    preview.addEventListener("ended", function () {
      // putar nyambung ala CapCut: lanjut ke klip berikutnya
      var i = -1;
      for (var k = 0; k < files.length; k++) { if (files[k].id === selectedId) { i = k; break; } }
      if (i >= 0 && i < files.length - 1) {
        previewFile(files[i + 1]);
        try { preview.play(); } catch (e) {}
      } else { setPlayIco(false); }
    });
  }
  ...
  function syncFullIco() {
    if (!fullIco) return;
    var fs = false;
    try { fs = !!(document.fullscreenElement || document.webkitFullscreenElement); } catch (e) {}
  ...
  try { document.addEventListener("fullscreenchange", syncFullIco); } catch (e) {}
  try { document.addEventListener("webkitfullscreenchange", syncFullIco); } catch (e) {}
  // tong sampah merah menempel di kanan teks hero (dalam hero, setelah deskripsi)
  var trashBtn = null;
  function ensureTrash() {
    ...
    if (mergeOpen && !attached) {
      ...
      trashBtn.addEventListener("click", function () { if (!busy) mergeClearCache(); });
      hero.appendChild(trashBtn);
    } else if (!mergeOpen && trashBtn) {
      try { trashBtn.remove(); } catch (e) {}
      trashBtn = null;
    } else if (trashBtn) {
  ...
  ensureTrash();
  // modal ditutup (X/backdrop/Esc) -> hentikan pratinjau merge
  try {
    var modalCard = document.getElementById("toolModalCard");
    if (modalCard && typeof MutationObserver !== "undefined") {
      new MutationObserver(function () {
        if (modalCard.hidden) {
          try { if (preview && !preview.paused) preview.pause(); } catch (e) {}
          stopLoop();
        }
      }).observe(modalCard, { attributes: true, attributeFilter: ["hidden"] });
    }
  } catch (e) {}
  ...
  if (rulerEl) {
    ...
    rulerEl.addEventListener("keydown", function (e) {
      ...
      if (e.key === "Home") { e.preventDefault(); seekVirtual(0); }
      if (e.key === "End") { e.preventDefault(); seekVirtual(total); }
    });
  }
  window.addEventListener("resize", function () { renderRuler(totalDuration()); updatePlayhead(); });
