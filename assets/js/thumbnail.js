/* VelardTools thumbnail menu — clone persis dari D:\TES THUMBNAIL\online-video-thumbnail-generator\js\app.js (bekerja).
 * Hanya penyesuaian: query di-scope ke #toolPanel-thumbnail dulu (fallback global),
 * guard anti double-init, dan guard gtag. Logika snap/auto/zip 100% sama. */
(function () {
  if (window.__fraigoThumbInit) return;
  window.__fraigoThumbInit = true;
  ...
  function thumbNotify(msg) {
    try {
      ...
      if (toast) {
        ...
        thumbNotify._t = setTimeout(function () { toast.classList.remove("show"); }, 3500);
        return;
      }
    } catch (e) { /* ignore */ }
  ...
  window.fraigoGoToTime = function (t) { goToTime(video, t); };
  ...
  if (video) {
    ...
    video.addEventListener("loadedmetadata", function () {
      ...
      try { video.play(); video.pause(); } catch (e) { /* ignore */ }
      resize();
    }, false);
  }
  ...
  window.fraigoResize = resize;
  if (videow) videow.addEventListener("change", resize);
  if (slider && video) {
    slider.addEventListener("input", function () { goToTime(video, parseFloat(slider.value) || 0); });
    slider.addEventListener("mouseover", function () { this.title = this.value + "sec"; });
  }
  ...
  function snapPicture() {
    ...
    img.addEventListener("click", function () { selectImage(img); });
  ...
  function zipAllImages() {
    if (typeof JSZip === "undefined") { alert("JSZip belum termuat. Muat ulang halaman."); return; }
    var zip = new JSZip();
    var container = q("#outputs");
    ...
    zip.generateAsync({ type: "blob" }).then(function (content) {
      if (typeof saveAs !== "undefined") saveAs(content, "images.zip");
      else {
  ...
  function autoSnapPictureAfterPercent(percentage) {
    if (!video || !video.duration) { thumbNotify(thumbT("thumbNeedVideo", "Muat video terlebih dahulu.")); return; }
    clearInterval(snapProc);
    // jangan hapus tangkapan manual — auto snap menambah ke galeri yang ada
    ...
    snapProc = setInterval(function () {
      ...
      time += interval;
      if (time >= duration) { clearInterval(snapProc); }
  ...
  window.__fraigoStop = function () { clearInterval(snapProc); };
  ...
  function selectImage(img) {
    ...
    for (var index = 0; index < images.length; index++) {
      ...
      if (element != img) { element.classList.remove("selected"); }
    }
  ...
  function loadVideoFile() {
    ...
    hideWarn();
    if (fileInput) {
      if (video.objectURL && video.src) { try { URL.revokeObjectURL(video.src); } catch (e) { /* ignore */ } }
  ...
  window.fraigoLoadVideoURL = loadVideoURL;

  function showInput(id) {
    var a = document.getElementById("select-file");
    var b = document.getElementById("select-url");
    // scope ke panel dulu agar tidak menabrak id lain
  ...
  function savePicture(btn) {
    ...
    if (selected) {
      ...
      link.click();
      setTimeout(function () { link.style.display = "none"; }, 100);
    }
  }
  ...
  if (save) save.addEventListener("click", function () { savePicture(save); });
  if (saveall) saveall.addEventListener("click", zipAllImages);
  if (clear) clear.addEventListener("click", function () { clearInterval(snapProc); clearSnaps(); });

  function thumbClearCache() {
    clearInterval(snapProc);
    try { if (video) video.pause(); } catch (e) { /* ignore */ }
    try { if (video && video.objectURL && video.src) URL.revokeObjectURL(video.src); } catch (e) { /* ignore */ }
    if (video) {
      video.objectURL = false;
      try { video.removeAttribute("src"); video.load(); } catch (e) { /* ignore */ }
    }
    try { if (file) file.value = ""; } catch (e) { /* ignore */ }
    if (slider) { slider.value = "0"; slider.setAttribute("max", "100"); }
  ...
  window.addEventListener("qm:langchange", function () { hideWarn(); refreshResultLabel(); });
  var cacheBtn = document.getElementById("thumbCacheClear");
  if (cacheBtn) cacheBtn.addEventListener("click", thumbClearCache);
  refreshResultLabel();

  var urlRadio = document.getElementById("fraigo_url_radio") || document.getElementById("video_url");
  var fileRadio = document.getElementById("fraigo_file_radio") || document.getElementById("video_file");
  if (fileRadio) fileRadio.addEventListener("click", function () { showInput("select-file"); });
  if (urlRadio) urlRadio.addEventListener("click", function () { showInput("select-url"); });
      ...
      for (var index = 0; index < buttons.length; index++) {
        (function (element) {
          element.addEventListener("click", function () {
            ...
            try { gtag("event", category + "-" + id, {}); } catch (e) { /* ignore */ }
          });
        })(buttons[index]);
      }
    } catch (e) { /* ignore */ }
